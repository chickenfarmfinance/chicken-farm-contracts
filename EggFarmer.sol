...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.5.1/contracts/utils/EnumerableSet.sol";
...
// EggFarmer is the master of EGG. He can make EGG and he is a fair farmer.
//
...
contract EggFarmer is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. EGG to distribute per block.
        uint256 lastRewardBlock;  // Last block number that EGG distribution occurs.
        uint256 accEggPerShare; // Accumulated EGG per share, times 1e12. See below.
    }
    ...
    function pendingEggs(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 eggReward = multiplier.mul(eggPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        eggToken.mint(devaddr, eggReward.div(50));
        eggToken.mint(address(this), eggReward);
        pool.accEggPerShare = pool.accEggPerShare.add(eggReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to EGG Farmer for EGG allocation.
    ...
    // Safe EGG transfer function, just in case if rounding error causes pool to not have enough EGG.
